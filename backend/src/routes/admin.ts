...
router.get('/deposits', async (req: any, res) => {
  try {
    const deposits = await db.select({
      ...
      bankName: depositRequests.bankName,
      bankMethod: depositRequests.bankMethod,
  ...
  try {
    ...
    if (draw.ticketCount < 5) {
      ...
      for (const ticket of ticketsList) {
        if (ticket.userId) {
          await db.update(users)
            .set({ balance: sql`${users.balance} + ${draw.ticketPrice}` })
...
router.get('/withdraws', async (req: any, res) => {
  try {
...
router.patch('/withdraws/:id', async (req: any, res) => {
  try {
    ...
    const withdrawResult = await db.select()
      .from(withdrawRequests)
    ...
    await db.update(withdrawRequests)
