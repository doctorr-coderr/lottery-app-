...
import { users, depositRequests, tickets, draws, winners, notifications, withdrawRequests } from '../db/schema';
...
router.get('/withdraws', async (req, res) => {
  try {
    const withdraws = await db.select({
      ...
      adminNotes: withdrawRequests.adminNotes,
      createdAt: withdrawRequests.createdAt,
      updatedAt: withdrawRequests.updatedAt,
      user: { id: users.id, email: users.email }
    }).from(withdrawRequests).leftJoin(users, eq(withdrawRequests.userId, users.id))
...
router.patch('/withdraws/:id', async (req: any, res) => {
  try {
    const { id } = req.params;
    const { status, adminNotes } = req.body;
    if (!['approved', 'rejected'].includes(status)) return res.status(400).json({ message: 'Invalid status' });

    const withdrawResult = await db.select().from(withdrawRequests).where(eq(withdrawRequests.id, id)).limit(1);
    if (!withdrawResult.length) return res.status(404).json({ message: 'Withdraw request not found' });

    const withdraw = withdrawResult[0];

    await db.update(withdrawRequests).set({ status, adminNotes, updatedAt: new Date() }).where(eq(withdrawRequests.id, id));

    if (status === 'approved' && withdraw.userId) {
      await db.update(users)
        .set({ balance: sql`${users.balance} - ${withdraw.amount}` })
        .where(eq(users.id, withdraw.userId));

      await db.insert(notifications).values({
        userId: withdraw.userId,
        message: `Your withdraw request of ETB ${withdraw.amount} has been approved. Funds will be sent manually.`,
        type: 'withdraw'
      });
