...
// Add to your backend index.ts or create a debug route
...
app.get('/api/debug/deposit-images', async (req, res) => {
  try {
    ...
    // Check if files actually exist - PROPERLY handle the column values
    const depositsWithFileInfo = deposits.map(deposit => {
      // Extract the filename from the imageUrl (which is now a string value)
