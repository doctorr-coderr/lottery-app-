...
{/* Recent Winners Section with Scroll Animation */}
<div className="w-full bg-gradient-to-r from-yellow-400 via-orange-500 to-red-500 rounded-lg shadow-lg text-white p-4 relative overflow-hidden">
  <div className="relative z-10 flex flex-col items-center space-y-4">
    <h2 className="text-xl font-bold">Latest Winners</h2>

    <div className="overflow-hidden w-full max-w-md h-36 relative">
      <div
        className="absolute top-0 left-0 w-full transition-transform duration-500 ease-in-out"
        style={{ transform: `translateY(-${currentWinnerIndex * 100}%)` }}
      >
        {recentWinners.slice(-3).map((winner, idx) => (
          <div
            key={winner.id}
            className="bg-white bg-opacity-20 rounded-lg p-4 mb-2 text-center"
          >
            <div className="font-bold text-lg">
              {winner.userName || `User #${winner.userId?.slice(-8)}`}
            </div>
            <div className="text-sm opacity-90 mt-1">
              Won {formatCurrency(winner.prizeAmount)} in Draw #{winner.draw?.id?.slice(-6)}
            </div>
