...
const openImageModal = async (imageUrl: string, depositId: string) => {
  try {
    // Use the API endpoint instead of direct file access
    const imageResponse = await api.get(`/deposits/image/${depositId}`, {
            ...
            <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
              {deposits.map((deposit) => (
                <tr key={deposit.id}>
                  ...
                  <button
                onClick={() => openImageModal(deposit.imageUrl, deposit.id)}
