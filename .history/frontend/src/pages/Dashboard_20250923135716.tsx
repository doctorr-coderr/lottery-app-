...
const Dashboard: React.FC = () => {
    ...
    const fetchDashboardData = async () => {
      try {
        const [drawsResponse, winnersResponse, userResponse, statsResponse] = await Promise.all([
          ...
          api.get('/users/me'),
          api.get('/draws/stats') // We'll create this endpoint
        ]);
  ...
  const calculatePotentialWin = (draw: Draw) => {
    ...
    const participantCount = draw.ticketCount || 1; // Avoid division by zero
    const totalPool = participantCount * ticketPrice;
    const potentialWin = totalPool * 0.8; // 80% prize pool
    return potentialWin;
  };
  ...
  const getExcitementMessage = (draw: Draw) => {
    ...
    const potentialWin = calculatePotentialWin(draw);
  ...
  const nextDraw = upcomingDraws[0]; // Next upcoming draw
