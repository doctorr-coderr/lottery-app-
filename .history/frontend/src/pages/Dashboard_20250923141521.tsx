...
interface Winner {
  ...
  draw: {
    id: string;
    drawTime: string;
    drawName?: string; // Added draw name for better display
  };
}
...
const Dashboard: React.FC = () => {
  ...
  const nextWinner = () => {
    setCurrentWinnerIndex((prevIndex) => 
      prevIndex === recentWinners.length - 1 ? 0 : prevIndex + 1
    );
  };

  const prevWinner = () => {
    setCurrentWinnerIndex((prevIndex) => 
      prevIndex === 0 ? recentWinners.length - 1 : prevIndex - 1
    );
  };
  ...
  return (
    <div className="space-y-6">
      {/* Winners Celebration Banner - Only show if there are winners */}
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex-1 max-w-2xl mx-8">
                <div className="text-center">
                  ...
                  {/* Dots indicator for multiple winners */}
                  {recentWinners.length > 1 && (
                    ...
                    <div className="flex items-center space-x-4 mt-2">
                      ...
                      <div className="text-center">
                        <div className="text-2xl font-bold text-green-600">
                          {nextDraw.ticketCount ? (1 / nextDraw.ticketCount * 100).toFixed(1) : 100}%
                        </div>
            ...
            <div className="p-6">
              {recentWinners.length > 0 ? (
                <div className="space-y-3 max-h-80 overflow-y-auto">
                  {recentWinners.slice(0, 8).map((winner, index) => (
                    <div 
                      key={winner.id} 
                      className={`flex items-center justify-between p-3 rounded-lg transition-all ${
                        index === currentWinnerIndex 
                          ? 'bg-gradient-to-r from-green-100 to-blue-100 dark:from-green-900 dark:to-blue-900 border-2 border-green-300 dark:border-green-600' 
                          : 'bg-gray-50 dark:bg-gray-700'
                      }`}
                    >
                      <div className="flex items-center space-x-3">
                        <div className={`w-8 h-8 rounded-full flex items-center justify-center ${
                          index === 0 ? 'bg-yellow-100 text-yellow-600' : 'bg-gray-200 dark:bg-gray-600'
                        }`}>
                          {index === 0 ? '👑' : index + 1}
                        </div>
                        <div>
                          <div className="font-semibold text-gray-800 dark:text-white truncate max-w-[120px]">
                            {winner.userName || `User #${winner.userId.slice(-8)}`}
                          </div>
                          <div className="text-xs text-gray-600 dark:text-gray-400">
                            Draw #{winner.draw.id.slice(-6)}
                          </div>
                        </div>
                      </div>
                      <div className="text-right">
