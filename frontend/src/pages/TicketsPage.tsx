...
const calculatePotentialWin = (draw: Draw & { ticketCount?: number }) => {
  ...
  return (
    <div className="max-w-4xl mx-auto pt-5">
                ...
                <div key={draw.id} className={`p-4 rounded-lg border ${
                  ...
                  {index === 0 && (
                    <div className="mt-2 text-sm text-green-600 dark:text-green-400">
                      🚀 Latest draw! potential: {formatCurrency(calculatePotentialWin(draw))}
                    </div>
                  )}
                </div>
              ))}
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-7 mb-6">
            ...
            >
              {availableDraws.map((draw) => (
                <option key={draw.id} value={draw.id}>
                  Draw #{draw.id.slice(-6)} - ETB {draw.ticketPrice}
                </option>
              ))}
            </select>
          </div>
      ...
      <div className="bg-blue-50 dark:bg-gray-800 rounded-lg shadow p-9 md:mx-auto lg:mx-auto mx-[-40px] mb-[-30px]">
        <h3 className="text-xl font-bold text-gray-800 dark:text-white mb-4">My Tickets</h3>
        {tickets.length > 0 ? (
