...
import { Home, CreditCard, Ticket, Wallet, Wallet2, UserCog } from 'lucide-react';
  ...
  return (
    <div className="flex h-screen bg-gray-100 dark:bg-gray-900">
      ...
      <div className={`fixed inset-y-0 left-0 z-50 w-72 bg-gradient-to-b from-indigo-900 to-gray-900 transform transition-transform duration-300 ease-in-out lg:static lg:translate-x-0 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <nav className="mt-6 px-3">
          {navItems.map((item) => (
            <Link
              key={item.path}
              to={item.path}
              className={`relative flex items-center px-4 py-4 my-2 text-white rounded-xl transition-all duration-200 ${
                isActive(item.path) ? "bg-white/10 shadow-lg" : "hover:bg-white/5"
              }`}
            ...
            >
              ...
              <div
                className={`absolute inset-0 bg-gradient-to-r from-cyan-400/10 to-blue-500/10 rounded-xl transition-opacity duration-300 ${
          ...
          {user?.isAdmin && (
            <Link
              to="/admin"
              className={`relative flex items-center px-4 py-4 my-2 text-white rounded-xl transition-all duration-200 ${
                isActive("/admin") ? "bg-white/10 shadow-lg" : "hover:bg-white/5"
              }`}
            ...
            >
              ...
              <UserCog className="w-5 h-5 mr-4" />
              <span className="font-medium">Admin Panel</span>
              <div
                className={`absolute inset-0 bg-gradient-to-r from-cyan-400/10 to-blue-500/10 rounded-xl transition-opacity duration-300 ${
                  activeHover === "/admin" ? "opacity-100" : "opacity-0"
                }`}
              ></div>
            </Link>
          )}
        ...
        {/* Sidebar footer */}
        <div className="absolute bottom-0 w-full px-6 py-4 mb-5">
          <div className="flex items-center justify-between">
            <DarkModeToggle />
            <button
      ...
      <div className="flex flex-col flex-1 overflow-hidden">
        {/* Header */}
        <header className="flex items-center justify-between px-6 py-4 mb-0 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700">
          ...
          <div className="flex flex-col">
            ...
            <div className="flex items-center space-x-2 mt-1">
              <h2 className="text-xs font-semibold text-gray-600 dark:text-gray-300">Balance:</h2>
              <p className="text-xs font-bold text-green-600 dark:text-green-400">{formatCurrency(balance)}</p>
            </div>
          </div>
        </div>
        ...
        <main className="flex-1 overflow-x-hidden overflow-y-auto bg-gray-100 dark:bg-gray-900">
          <div className="container px-6 py-8 pt-0 mx-auto">
            {children}
          </div>
        </main>
      </div>
